...
public class CreateEventCommandParserTest {
    ...
    public void parse_repeatedNonTagValue_failure() {
        String validExpectedEventString = EVENT_NAME_CAREER_FAIR
                + EVENT_DESC_CAREER_FAIR + EVENT_DT_CAREER_FAIR;
        ...
        // multiple dates
        assertParseFailure(parser, EVENT_DT_FSC + validExpectedEventString,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_EVENT_DATE));

        // multiple fields repeated
        assertParseFailure(parser,
                validExpectedEventString + EVENT_DESC_FSC + EVENT_DT_FSC + EVENT_NAME_FSC
                        + validExpectedEventString,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_EVENT_NAME, PREFIX_EVENT_DESC, PREFIX_EVENT_DATE));
        ...
        // invalid phone
        assertParseFailure(parser, INVALID_EVENT_DATE + validExpectedEventString,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_EVENT_DATE));
        ...
        // invalid phone
        assertParseFailure(parser, validExpectedEventString + INVALID_EVENT_DATE,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_EVENT_DATE));
    }
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_EVENT_NAME_FSC + EVENT_DESC_FSC + EVENT_DT_FSC,
                expectedMessage);

        // missing phone prefix
        assertParseFailure(parser, EVENT_NAME_FSC + VALID_EVENT_DESCRIPTION_FSC + EVENT_DT_FSC,
                expectedMessage);

        // missing email prefix
        assertParseFailure(parser, EVENT_NAME_FSC + EVENT_DESC_FSC + VALID_EVENT_DATE_FSC,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_EVENT_NAME_FSC + VALID_EVENT_DESCRIPTION_FSC + VALID_EVENT_DATE_FSC,
                expectedMessage);
    }
