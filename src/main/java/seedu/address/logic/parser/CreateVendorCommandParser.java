...
public class CreateVendorCommandParser implements Parser<CreateVendorCommand> {
    ...
    public CreateVendorCommand parse(String args) throws ParseException {
        ...
        Name name = ParserUtil.parseVendorName(argMultimap.getValue(PREFIX_NAME).get());
        Phone phone = ParserUtil.parseVendorPhone(argMultimap.getValue(PREFIX_PHONE).get());
        Email email = ParserUtil.parseVendorEmail(argMultimap.getValue(PREFIX_EMAIL).get());

        Vendor vendor = new Vendor(name, phone, email);
